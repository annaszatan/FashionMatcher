...
from ml.models.components.backbone import build_retrieval_model
...
def main():
    ...
    # Build model and load saved checkpoint
    ...
    # Get metadata and image paths for dataset
    normalize = bool(cfg.get("model", {}).get("normalize_embeddings", True))
    metadata_path = resolve(cfg["paths"]["metadata"], PROJECT_ROOT)
    image_root = resolve(cfg["paths"]["image_root"], PROJECT_ROOT)
    ...
    features_dir = resolve(features_dir, PROJECT_ROOT)
    ...
    # Go through both splits and extract + save features
